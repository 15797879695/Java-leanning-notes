...
Spring Boot 通过`@EnableAutoConfiguration`开启自动装配，实现AutoConfigurationImportSelector 类，AutoConfigurationImportSelector 类又就实现ImportSelecter 接口的ImportSelect（） 方法，作用是**获取所有符合条件的类的全限定类名，这些类需要被加载到 IoC 容器中**，此方法又实现getAutoConfigurationEntry（）作用是获取自动装配类，通过 SpringFactoriesLoader 最终加载`META-INF/spring.factories`中的自动配置类实现自动装配，自动配置类其实就是通过`@Conditional`按需加载的配置类，想要其生效必须引入`spring-boot-starter-xxx`包实现起步依赖。